...
var ErrorLogger = func() *log.Logger {
	...
	if os.Getenv("DEVELOPMENT") == "true" {
		file, err := os.OpenFile("data/logs/errors.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0666)
		if err != nil {
			log.Fatalf("Failed to open log file: %v", err)
		}
		return log.New(file, "ERROR: ", log.Ldate|log.Ltime|log.Lshortfile)
	}
	os.Remove("data/logs/errors.log")
...
var InfoLogger = func() *log.Logger {
	...
	if os.Getenv("DEVELOPMENT") == "true" {
		file, err := os.OpenFile("data/logs/info.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0666)
		if err != nil {
			log.Fatalf("Failed to open log file: %v", err)
		}
		return log.New(file, "INFO: ", log.Ldate|log.Ltime)
	}
	os.Remove("data/logs/info.log")
	return log.New(io.Discard, "", 0)
}()
